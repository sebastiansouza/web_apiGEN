...
@router.post("/themes/", status_code=status.HTTP_201_CREATED, response_model=Theme)
def create_theme(theme: ThemeCreate, db: Session = Depends(get_db)):
    return theme_crud.create_theme(db=db, theme=theme)

@router.get("/themes/", status_code=status.HTTP_200_OK, response_model=List[Theme])
...
@router.get("/themes/{theme_id}", status_code=status.HTTP_200_OK, response_model=Theme)
...
@router.put("/themes/{theme_id}", status_code=status.HTTP_200_OK, response_model=Theme)
...
@router.delete("/themes/{theme_id}", status_code=status.HTTP_200_OK, response_model=Theme)
